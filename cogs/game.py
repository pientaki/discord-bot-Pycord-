    ...
    async def guess(self, ctx: discord.ApplicationContext):
        ...
        try:
            ...
            async with ctx.typing():
                ...
                await ctx.respond(embed=embed_question)
                ...
                while aki.progression <= 85:
                    ...
                    try:
                        ...
                        embed_game_ended = discord.Embed(
                            ...
                            color=discord.Colour.from_rgb(255, 208, 0))
                        await ctx.respond(embed=embed_game_ended)
                        return
                    ...
                    if str(symbol) == emojis_c[0]:
                        ...
                        embed_game_end = discord.Embed(
                            title='ゲームを終了します',
                            color=discord.Colour.from_rgb(255, 208, 0))
                        await ctx.respond(embed=embed_game_end)
                        return
                ...
                try:
                    ...
                    embed_loss = discord.Embed(title=t_loss,
                                            description=d_loss,
                                            color=discord.Colour.from_rgb(255, 208, 0))
                    await ctx.respond(embed=embed_loss)
                    return
                ...
                if str(s) == emojis_w[0]:
                    ...
                    await ctx.respond(embed=embed_win)
    ...
    async def on_command_error(self, ctx: discord.ApplicationContext, error):
        if isinstance(error, commands.MaxConcurrencyReached):
            ...
            embed_var_four = discord.Embed(title=title_error_four,
                                        ...
                                        color=discord.Colour.from_rgb(255, 208, 0))
            await ctx.respond(embed=embed_var_four)
...
#マインスイーパー
    ...
    async def minesweeper(self, ctx: discord.ApplicationContext, columns: Option(str, '縦のマス数'), rows: Option(str, '横のマス数'), bombs: Option(str, '爆弾の数')):
        ...
        if columns > 13 or rows > 13:
            await ctx.respond('**ERROR**  縦のマス数は13が限界です....')
            return
        if columns < 1 or rows < 1 or bombs < 1:
            await ctx.respond('**ERROR**  マス数が少なすぎます...')
            return
        if bombs + 1 > columns * rows:
            await ctx.respond('**ERROR**  :boom:マス数より多く爆弾の数が設定されています')
            return
